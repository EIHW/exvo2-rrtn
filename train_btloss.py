...
from Metrics import UncertaintyRevised
from dataloador.Exvo_A import CachedDataset, CCCLoss
from models.vmodels import ResNet14_BT
...
def training_setting(model, lr=1e-4):
    paras = list(model.parameters()) + list(loss_ratio.parameters())

    optimizer = AdamW(paras, lr=lr)
    loss_fn = CCCLoss()
...
def train(dl, optimizer, loss_fn, epoch, log_freq=10):
    print('loss_ratio: {}'.format(loss_ratio.log_vars))
    losses = 0.
    brl_losses = 0.
    counter = 1
    tmp_losses = 0.
    ...
    for idx, (batch, label) in enumerate(dl):
        ...
        out1, out2, btl = model(batch)
        del batch
        optimizer.zero_grad()
        loss1 = loss_fn(out1, label)
        loss2 = loss_fn(out2, label)
        loss = loss_ratio(loss1, loss2, btl[0])
        loss.backward()
        optimizer.step()

        losses += loss.item()
        brl_losses += btl[0].item()
        counter += 1

        tmp_losses += loss1.item()
        tmp_counter += 1
        ...
        pred.append(out1.detach().cpu().numpy())
    ...
    print('##>[{}]Train Results: {:4f}, brl: {:4f}'.format(epoch, np.mean(ccc_result), brl_losses / counter))
...
if __name__ == "__main__":
    ...
    if not tr_cfg.resume:
        model = ResNet14_BT(num_classes=10)
        # model = Cnn14(10)
        # model = Cnn10_bt(10, False)
